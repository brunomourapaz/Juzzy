...
//    private byte inferenceMethod = 0;  //not used ATM
	...
	/**
	 * This method assumes all rules use the same (and all) inputs. The first rule
	 * is queried to identify the inputs and return them.
	 * 
	 * @return An array of the inputs used in the rulebase (retrieved from the
	 *         actecedents of the firts rule in the rulebase!).
	 */
	 ...
	 *         where Object[0] is a Tuple (the centroid) and Object[1] is a Double
	 *         holding the associated yValue for the centroid. If not rule fired for
	 *         the given input(s), then null is returned as an Object[].
	 */
	...
	/**
	 * Returns typereduced & defuzzified result of evaluating all rules in the
	 * rulebase.
	 * 
	 * @param typeReductionType   The type of type reducer to be used:
	...
	public TreeMap<Output, Tuple> doCOSTypeReduction() {
		...
		if (data.firstEntry() == null || ((Vector<IT2_COSInferenceData>) data.firstEntry().getValue()[0]).size() == 0) { // no
																															// rules
																															// fired
			while (oIt.hasNext())
			...
			while (oIt.hasNext()) {
...

				{
					...
					compute_yr: while (!stopFlag) {
						// Step2
						for (int i = 0; i < fir.length - 1; i++) // set to rulesFired-2
						{
							// System.out.println("yDash = "+yDash+" and rCCRight[i] = "+ rCCRight[i]);
							if (rightData[i].getSelectedCentroidEndpoint() <= yDash
									&& yDash <= rightData[i + 1].getSelectedCentroidEndpoint()) {
								R = i;
								break;
							}
						}
						...
						if (Math.abs(yDash - yDashDash) < 0.000000001)// watch out for problem with doubles use
																		// difference of absolute value < 0.003...
						{
							stopFlag = true;
							yDashDash = yr;
						} else {
					...
					yDash = yl;
					if (DEBUG)
						System.out.println("Intitial yDash for left = " + yDash);

					compute_yl: while (!stopFlag) {
						// Step2
						for (int i = 0; i <= fir.length - 2; i++) // set to rulesFired-2
						{
							if (leftData[i].getSelectedCentroidEndpoint() <= yDash
									&& yDash <= leftData[i + 1].getSelectedCentroidEndpoint()) {
								L = i;
								break;
							}
						}
						...
						// Step4
						if (DEBUG)
							System.out.println("yDash = " + yDash + " and yDashDash = " + yDashDash);

						// difference smaller than 1 in a billion
						if (Math.abs(yDash) - Math.abs(yDashDash) < 0.000000001)// watch out for problem with doubles
																				// use difference of absolute value <
																				// 0.003...
						{
							stopFlag = true;
							yDashDash = yl;
					...
					if (DEBUG)
						System.out.println("returning yl = " + yl + " and yr= " + yr);
	...
	private synchronized TreeMap<Output, Object[]> getFiringIntervalsForCOS_TR() {
		TreeMap<Output, Object[]> returnValue = new TreeMap();
		if (DEBUG)
			System.out.println("Number of rules in rulebase: " + rules.size());
			...
			if (firingStrength.getRight() > 0.0) // check if rule fired
			{
				while (ruleCons.hasNext()) {
					...
					if (!returnValue.containsKey(cons.getOutput())) {
						returnValue.put(cons.getOutput(), new Object[] { new Vector<IT2_COSInferenceData>(),
								new Vector<IT2_COSInferenceData>() }); // for left and right consequentData object -
																		// needed for separate sorting
					}
					((Vector) returnValue.get(cons.getOutput())[0]).add(new IT2_COSInferenceData(firingStrength,
							temp.getConsequentCentroid(cons.getOutput()).getLeft()));
					((Vector) returnValue.get(cons.getOutput())[1]).add(new IT2_COSInferenceData(firingStrength,
							temp.getConsequentCentroid(cons.getOutput()).getRight()));
				}
			}
			ruleCounter++;
		}
			...
			if (fStrength.getRight() > 0.0) {
				...
				while (itC.hasNext()) {
					...
					if (firstFiredForOutput.get(o)) {
						overallOutputSet.put(o,
								new IntervalT2MF_Intersection(new IntervalT2MF_Cylinder("FiringInterval", fStrength),
										c.getMembershipFunction(), getTypeIntersection()));
						...
						if (overallOutputSet.get(o) == null) {
							...
							overallOutputSet.put(o,
									new IntervalT2MF_Union(
											new IntervalT2MF_Intersection(
													new IntervalT2MF_Cylinder("FiringInterval", fStrength),
													c.getMembershipFunction(), getTypeIntersection()),
											overallOutputSet.get(o), getTypeUnion()));
						}
					}
				}
			}
...
//    }
	 ...
	 * of the AND logical connective between parts of the antecedent. The desired
	 * implication method is applied for all rules.
	 * 
